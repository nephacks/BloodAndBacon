...
namespace Blood
{
  internal class Princess4
  {
    ...
    public Princess4.conductor tempConduct = new Princess4.conductor();
    private Princess4.conductor cuttyStruct = new Princess4.conductor();
    private Princess4.conductor cuttyStruct_send = new Princess4.conductor();
    ...
    public float[] distanceCutty2 = new float[6]
    {
      100000f,
      100000f,
      100000f,
    ...
    public Princess4(int index, string name, bool thisTrial)
    {
      ...
      Princess4.bit = (byte) 0;
      Princess4.uvIndex = (byte) 0;
      Princess4.xcoord = 0;
      Princess4.ycoord = 0;
    ...
    public void LoadContent(ScreenManager sc, ContentManager cc)
    {
      ...
      this.puke1 = new Princess4.vomit();
      this.puke1.max = 0;
      ...
      this.bloody = new Princess4.finale();
      this.bloody.max = 0;
      ...
      this.chunk = new Princess4.shell();
      this.chunk.max = 0;
      ...
      this.faceChunk = new Princess4.shell();
      ...
      this.assChunk = new Princess4.shell();
      this.assChunk.max = 0;
    ...
    public void SetTangents(ref Curve x, ref Curve y)
    {
      for (int index1 = 0; index1 < x.Keys.Count; ++index1)
      {
        ...
        Princess4.SetCurveKeyTangent(ref key1, ref key3, ref key2);
        x.Keys[index1] = key3;
        CurveKey key4 = y.Keys[index2];
        CurveKey key5 = y.Keys[index3];
        CurveKey key6 = y.Keys[index1];
        Princess4.SetCurveKeyTangent(ref key4, ref key6, ref key5);
        y.Keys[index1] = key6;
      }
    }
      ...
      int bonehit)
    {
      ...
      Princess4.xcoord = (int) num2;
      Princess4.ycoord = (int) num3;
      int index = 6;
      if ((double) this.distanceCutty < (double) this.minDistance[this.df])
    ...
    private void updatePuke(ref Princess4.vomit sh, ref float[,] heights)
    {
      ...
      if (sh.max <= 50)
        return;
      for (int index1 = 10; index1 < sh.max; index1 += 60)
      {
        if (!this.cuttyVomitHit && sh.dupe[index1].move == 1 && (double) Vector3.DistanceSquared(sh.dupe[index1].mypos, new Vector3(this.playerpos.X, this.playerpos.Y + 50f, this.playerpos.Z)) < 8100.0)
        {
          this.cuttyVomitHit = true;
          this.splats.Play(this.sc.ev, (float) this.rr.Next(-50, 20) / 100f, (float) this.rr.Next(-90, 90) / 100f);
        }
        if (this.cuttyVomitHit2 == -1 && sh.dupe[index1].move == 1 && !this.Alone && this.remplayer.Count > 1)
        {
          for (int index2 = 0; index2 < this.remplayer.Count; ++index2)
          {
            if (index2 != this.playerIndex && (double) Vector3.DistanceSquared(sh.dupe[index1].mypos, this.remplayer[index2].r.lastPOS + new Vector3(0.0f, 50f, 0.0f)) < 8100.0)
              this.cuttyVomitHit2 = index2;
          }
        }
        if (sh.dupe[index1].splat)
        {
          sh.dupe[index1].splat = false;
          this.addCanvasCrap(this.rr.Next(1, 4), sh.dupe[index1].mypos.X, sh.dupe[index1].mypos.Z);
        }
      }
    }
    ...
    private void updateShit(ref Princess4.vomit sh, ref float[,] heights)
    {
      ...
      for (int index1 = 10; index1 < sh.max; index1 += 60)
      {
        if (!this.cuttyShitHit && sh.dupe[index1].move == 1 && (double) Vector3.DistanceSquared(sh.dupe[index1].mypos, new Vector3(this.playerpos.X, this.playerpos.Y + 60f, this.playerpos.Z)) < 14400.0)
        {
          this.cuttyShitHit = true;
          this.splats.Play(this.sc.ev, (float) this.rr.Next(-99, -30) / 100f, (float) this.rr.Next(-90, 90) / 100f);
        }
        if (this.cuttyShitHit2 == -1 && sh.dupe[index1].move == 1 && !this.Alone && this.remplayer.Count > 1)
        {
          for (int index2 = 0; index2 < this.remplayer.Count; ++index2)
          {
            if (index2 != this.playerIndex && (double) Vector3.DistanceSquared(sh.dupe[index1].mypos, this.remplayer[index2].r.lastPOS + new Vector3(0.0f, 60f, 0.0f)) < 14400.0)
              this.cuttyShitHit2 = index2;
          }
        }
        if (sh.dupe[index1].splat)
        {
          sh.dupe[index1].splat = false;
          this.addCanvasCrap(this.rr.Next(9, 13), sh.dupe[index1].mypos.X, sh.dupe[index1].mypos.Z);
        }
      }
    }
          ...
          else
          {
            ...
            if (!this.gonnaShit && (this.gonnaVomit || this.homingTimer > 0 || (double) this.shitDelay > 0.0 || this.cuttyShit || this.attack1))
            {
              ...
              else
              {
                ...
                if (this.Alone)
                {
                  if ((double) this.distanceCutty < (double) num1)
                ...
                if (!this.Alone)
                {
                  for (int index2 = 0; index2 < this.remplayer.Count; ++index2)
                  {
                    if (index2 != this.playerIndex)
                    {
                      if ((double) this.remplayer[index2].r.now.health > 0.0 && (double) this.distanceCutty2[index2] < (double) num1)
                        intList1.Add(index2);
                      else
                        intList2.Add(index2);
                    }
                    else if ((double) this.playerHealth > 0.0 && (double) this.distanceCutty < (double) num1)
                      intList1.Add(this.playerIndex);
                    else
                      intList2.Add(this.playerIndex);
                  }
                  if (intList1.Count > 0)
                    index1 = intList1[this.rr.Next(0, intList1.Count)];
                  else if (intList2.Count > 0)
                    index1 = intList2[this.rr.Next(0, intList2.Count)];
                  if (intList2.Count > 0 && this.rr.Next(1, 1000) < 200)
                    index1 = intList2[this.rr.Next(0, intList2.Count)];
                }
                if (index1 > -1)
                {
                  if (flag2)
                  {
                    ...
                    float radians = index1 != this.playerIndex ? (float) (-Math.Atan2((double) this.cuttyPos.Z - (double) this.remplayer[index1].r.lastPOS.Z, (double) this.cuttyPos.X - (double) this.remplayer[index1].r.lastPOS.X) - 1.5700000524520874) : (float) (-Math.Atan2((double) this.cuttyPos.Z - (double) this.playerpos.Z, (double) this.cuttyPos.X - (double) this.playerpos.X) - 1.5700000524520874);
                    float num6 = Math.Abs(Princess4.WrapAngle(radians) - Princess4.WrapAngle(this.cuttyRot));
                    this.cuttyStruct.animType = 1;
                    float num7 = 0.5f;
                    if ((double) Princess4.WrapAngle(radians - this.cuttyRot) < 0.0)
                    {
                      this.cuttyStruct.animType = 2;
                      num7 = -0.5f;
                    }
                    this.cuttyStruct.homing = index1 + 1;
                    this.cuttyStruct.rot = radians;
                    if ((double) num6 < 2.5)
                      this.cuttyStruct.rot = radians + num7;
                    this.cuttyStruct.talkindex = -1;
                  ...
                  else if (flag3)
                  {
                    ...
                    {
                      radians = (float) (-Math.Atan2((double) this.cuttyPos.Z - (double) this.remplayer[index1].r.lastPOS.Z, (double) this.cuttyPos.X - (double) this.remplayer[index1].r.lastPOS.X) - 4.7119998931884766);
                      if ((double) this.remplayer[index1].r.now.health < 100.0)
                        flag4 = true;
                    }
                    float num8 = Math.Abs(Princess4.WrapAngle(radians) - Princess4.WrapAngle(this.cuttyRot));
                    float num9 = 0.3f;
                    this.cuttyStruct.animType = 1;
                    if ((double) Princess4.WrapAngle(radians - this.cuttyRot) < 0.0)
                    {
                    ...
                    this.cuttyStruct.rot = radians;
                    if ((double) num8 < 2.5 && !flag4)
                      this.cuttyStruct.rot = radians + num9;
                    this.cuttyStruct.talkindex = -1;
                    this.scheduleAction(140);
                    this.newAction = true;
                  }
                  else
                  {
                    this.homing = 0;
                    float radians = index1 != this.playerIndex ? (float) (-Math.Atan2((double) this.cuttyPos.Z - (double) this.remplayer[index1].r.lastPOS.Z, (double) this.cuttyPos.X - (double) this.remplayer[index1].r.lastPOS.X) - 1.5700000524520874) : (float) (-Math.Atan2((double) this.cuttyPos.Z - (double) this.playerpos.Z, (double) this.cuttyPos.X - (double) this.playerpos.X) - 1.5700000524520874);
                    float num10 = Math.Abs(Princess4.WrapAngle(radians) - Princess4.WrapAngle(this.cuttyRot));
                    float num11 = 0.5f;
                    this.cuttyStruct.animType = 1;
                    if ((double) Princess4.WrapAngle(radians - this.cuttyRot) < 0.0)
                    {
                    ...
                    this.cuttyStruct.rot = radians;
                    if ((double) num10 < 2.5)
                      this.cuttyStruct.rot = radians + num11;
                    this.cuttyStruct.talkindex = -1;
                    this.scheduleAction(140);
                    this.newAction = true;
                  }
    ...
    private void pigBones(Vector3 lookVec, ref float[,] heights)
    {
      ...
      this.distanceCutty2 = new float[5]
      {
        100000f,
        100000f,
        100000f,
      ...
      if (this.remplayer.Count > 1)
      {
        for (int index = 0; index < this.remplayer.Count; ++index)
        {
          if (index != this.playerIndex)
            this.distanceCutty2[index] = Vector2.DistanceSquared(new Vector2(this.remplayer[index].r.lastPOS.X, this.remplayer[index].r.lastPOS.Z), new Vector2(this.cuttyPos.X, this.cuttyPos.Z));
        }
      }
      if (this.attack1)
      ...
      if (this.homingTimer > 0)
      {
        ...
        int index = this.homing - 1;
        if (index == this.playerIndex)
        {
          y = this.bonePos.Z - this.playerpos.Z;
          x = this.bonePos.X - this.playerpos.X;
        }
        else if (this.remplayer.Count > 0 && index >= 0 && index < this.remplayer.Count)
        {
          y = this.bonePos.Z - this.remplayer[index].r.lastPOS.Z;
          x = this.bonePos.X - this.remplayer[index].r.lastPOS.X;
        }
      ...
      {
        this.wehurtBoss = 0;
        float num2 = Princess4.WrapAngle(num1 - this.piglook);
        this.piglook = Princess4.WrapAngle(this.piglook + ((double) num1 != 0.0 ? MathHelper.Clamp(num2, -0.06f, 0.06f) : MathHelper.Clamp(num2, -0.06f, 0.06f)));
      ...
      if (!this.cuttyisDead)
      {
        ...
        this.cuttyRot = Princess4.WrapAngle(this.cuttyRot + MathHelper.Clamp(Princess4.WrapAngle(num8 - this.cuttyRot), -max, max));
        if (this.homingDirection > 0 && (double) Math.Abs(Princess4.WrapAngle(this.targetRot - this.cuttyRot)) <= 0.019999999552965164)
        {
    ...
    {
      int index1 = (int) MathHelper.Clamp(position.X / Princess4.unit, 0.0f, (float) (Princess4.bitmap - 2));
      int index2 = (int) MathHelper.Clamp(position.Y / Princess4.unit, 0.0f, (float) (Princess4.bitmap - 2));
    ...
    public struct instancedObject : IVertexType
    {
      ...
      {
        get => Princess4.instancedObject.InstanceVertexDeclaration;
      }
    }
