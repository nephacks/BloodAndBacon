...
namespace Blood
{
  public class caldDupe
  {
    ...
    public caldDupe(int i) => this.rr = new Random(i);
    ...
    private static void GetHeightSlow(
      ...
      out Vector3 normal)
    {
      int index1 = (int) MathHelper.Clamp(pos.X / caldDupe.unit, 0.0f, (float) (caldDupe.bitmap - 2));
      int index2 = (int) MathHelper.Clamp(pos.Z / caldDupe.unit, 0.0f, (float) (caldDupe.bitmap - 2));
      float num1 = pos.X % caldDupe.unit / caldDupe.unit;
      float num2 = pos.Z % caldDupe.unit / caldDupe.unit;
