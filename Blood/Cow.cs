...
namespace Blood
{
  internal class Cow
  {
    ...
    private Cow.hitStream hitstreamTemp = new Cow.hitStream();
    ...
    public Cow.conductor tempConduct = new Cow.conductor();
    private Cow.conductor cuttyStruct = new Cow.conductor();
    private Cow.conductor cuttyStruct_send = new Cow.conductor();
    ...
    private int[] col_Bone = new int[4]{ 2, 4, 5, 8 };
    ...
    public void LoadContent(ScreenManager sc, ContentManager cc)
    {
      ...
      this.pig1[0].StartClip(tag.AnimationClips["cowrun"]);
      this.pig1[1] = new AnimationPlayer(tag);
      this.pig1[1].StartClip(tag.AnimationClips["cowwalk"]);
      this.pig1[2] = new AnimationPlayer(tag);
      this.pig1[2].StartClip(tag.AnimationClips["cowwait"]);
      this.pig1[3] = new AnimationPlayer(tag);
      this.pig1[3].StartClip(tag.AnimationClips["cowbite"]);
      this.pig1[4] = new AnimationPlayer(tag);
      this.pig1[4].StartClip(tag.AnimationClips["cowdie"]);
      this.pig1[5] = new AnimationPlayer(tag);
      this.pig1[5].StartClip(tag.AnimationClips["cowjump"]);
      this.pig1[6] = new AnimationPlayer(tag);
      this.pig1[6].StartClip(tag.AnimationClips["cowheal"]);
      this.a = this.pig1[0];
      this.b = this.pig1[0];
      this.clipIndexA = 0;
      ...
      this.col_Bone = new int[4]{ 2, 4, 5, 8 };
      this.col_Scale[0] = 18.5f;
      this.col_Pos[0] = new Vector3(0.0f, 0.094f, -0.045f);
      this.col_Scale[1] = 20f;
      this.col_Pos[1] = new Vector3(0.0f, 0.084f, 0.01f);
      this.col_Scale[2] = 17f;
      this.col_Pos[2] = new Vector3(0.0f, 0.099f, 0.066f);
      this.col_Scale[3] = 14f;
      this.col_Pos[3] = new Vector3(0.0f, 0.126f, 0.12f);
    ...
    public void SetTangents(ref Curve x, ref Curve y)
    {
      for (int index1 = 0; index1 < x.Keys.Count; ++index1)
      {
        ...
        Cow.SetCurveKeyTangent(ref key1, ref key3, ref key2);
        x.Keys[index1] = key3;
        CurveKey key4 = y.Keys[index2];
        CurveKey key5 = y.Keys[index3];
        CurveKey key6 = y.Keys[index1];
        Cow.SetCurveKeyTangent(ref key4, ref key6, ref key5);
        y.Keys[index1] = key6;
      }
    }
    ...
    public void checkTargets(Vector3 gunpos, Vector3 gunlook, ref Cursor genCursor, float wall)
    {
      ...
      for (int index = 0; index < this.col_Bone.Length; ++index)
      {
        ...
        float? nullable = genCursor.hitSphere2(gunpos, gunlook, cent, this.col_Scale[index]);
      ...
      if (this.boneHit == 0)
      {
        ...
        if ((double) vector3.X < 0.0)
          this.addBulletHole(new Vector2(14f, 509f), new Vector2(401f, 456f), new Vector2(41f, 128f), new Vector2(399f, 143f), vector3.Z, vector3.Y, this.boneHit);
        else
          this.addBulletHole(new Vector2(994f, 513f), new Vector2(618f, 454f), new Vector2(1016f, 100f), new Vector2(609f, 180f), vector3.Z, vector3.Y, this.boneHit);
      }
      else if (this.boneHit == 1)
      {
        Vector3 vector3 = Vector3.Transform(this.hitEdge - this.hitCenter, Matrix.CreateRotationY(-this.cuttyRot));
        if ((double) vector3.X < 0.0)
          this.addBulletHole(new Vector2(55f, 768f), new Vector2(412f, 600f), new Vector2(12f, 356f), new Vector2(410f, 399f), vector3.Z, vector3.Y, this.boneHit);
        else
          this.addBulletHole(new Vector2(968f, 767f), new Vector2(602f, 597f), new Vector2(1009f, 360f), new Vector2(614f, 395f), vector3.Z, vector3.Y, this.boneHit);
      }
      else if (this.boneHit == 2)
      {
        Vector3 vector3 = Vector3.Transform(this.hitEdge - this.hitCenter, Matrix.CreateRotationY(-this.cuttyRot));
        if ((double) vector3.X < 0.0)
          this.addBulletHole(new Vector2(141f, 890f), new Vector2(464f, 758f), new Vector2(74f, 638f), new Vector2(400f, 544f), vector3.Z, vector3.Y, this.boneHit);
        else
          this.addBulletHole(new Vector2(878f, 898f), new Vector2(564f, 736f), new Vector2(962f, 640f), new Vector2(612f, 546f), vector3.Z, vector3.Y, this.boneHit);
      }
      else if (this.boneHit == 3)
      {
        Vector3 vector3 = Vector3.Transform(this.hitEdge - this.hitCenter, Matrix.CreateRotationY(-this.cuttyRot));
        if ((double) vector3.X < 0.0)
          this.addBulletHole(new Vector2(402f, 1005f), new Vector2(522f, 985f), new Vector2(260f, 846f), new Vector2(516f, 773f), vector3.Z, vector3.Y, this.boneHit);
        else
          this.addBulletHole(new Vector2(797f, 984f), new Vector2(515f, 998f), new Vector2(580f, 701f), new Vector2(509f, 749f), vector3.Z, vector3.Y, this.boneHit);
      }
      ushort amt = 0;
      ...
      int hit)
    {
      ...
      Cow.xcoord = (int) num2;
      Cow.ycoord = (int) num3;
      this.addTargetBlood(this.sc.paintColor, (int) num2 - 35, (int) num2 + 35, (int) num3 - 35, (int) num3 + 35);
    }
    ...
    private void pigLogic()
    {
      ...
      else
      {
        ...
        if (!this.newAction && Cow.gonnaHealindex == -1)
        {
          ...
          if (this.healCount > 0 && (flag1 || flag2) && (double) this.gonnaHealDelay <= 0.0)
          {
            ...
            if ((double) num3 <= (double) num1)
            {
              ...
              float num7 = Cow.WrapAngle(y - num6);
              float num8 = Cow.WrapAngle(y + 3.14f - num6);
        ...
        if (num9 < this.break1)
        {
          ...
          if ((double) this.playerHealth > 0.0 && Cow.homingLocal == -1)
        ...
        if (num9 >= this.break2)
        {
          ...
          this.cuttyStruct.targetRate = (float) ((double) this.rr.Next(90, 130) / 100.0 * 1.0);
        ...
        else if (num9 >= this.break1)
        {
          ...
          this.cuttyStruct.homing = 0;
          this.cuttyStruct.curveIndex = this.curveIndex;
          this.cuttyStruct.targetRate = (float) ((double) this.rr.Next(80, 130) / 100.0 * 1.0);
          this.cuttyStruct.loop = this.loop;
          this.cuttyStruct.animType = 0;
    ...
    private void pigBones(Vector3 lookVec, ref float[,] heights)
    {
      ...
      this.piglook = Cow.WrapAngle(this.piglook + MathHelper.Clamp(Cow.WrapAngle(num2 - this.piglook), -0.013f, 0.013f));
      ...
      if (this.isSpeaking)
        this.a.boneTransforms[12] = Matrix.CreateRotationZ(MathHelper.ToRadians(this.talkSmooth - 4f)) * this.a.boneTransforms[12];
      if (!this.cuttyisDead)
      {
        string type = "";
        ...
        float num5 = Cow.WrapAngle((float) (-Math.Atan2((double) this.cuttyPos.Z - (double) this.tz, (double) this.cuttyPos.X - (double) this.tx) - 1.5700000524520874) - this.cuttyRot);
        float max = 0.065f;
        if (this.homing > 0)
        {
          max = 0.065f;
          num5 = MathHelper.Clamp(num5, -0.065f, max);
          this.cuttyRot = Cow.WrapAngle(this.cuttyRot + num5);
        }
        if (this.attack1)
        {
          max = 0.06f;
        ...
        if (this.cuttyHeal)
        {
          ...
          {
            num5 = Cow.WrapAngle(this.cuttyHealrot - this.cuttyRot);
            max = 0.01f;
          }
        ...
        if (!this.death1 && !this.cuttyHeal && !this.attack1 && this.homing == 0)
          this.cuttyRot = Cow.WrapAngle(this.cuttyRot + MathHelper.Clamp(num5, -max, max));
    ...
    private void pigClipControl()
    {
      ...
      if ((double) this.cuttyRate <= 0.0)
        this.myClip = 2;
      if ((double) this.cuttyRate > 0.0 && (double) this.cuttyRate <= 5.0)
        this.myClip = 1;
    ...
    private void pigDying()
    {
      ...
      else
      {
        ...
        this.cuttyRot = Cow.WrapAngle(this.cuttyRot + MathHelper.Clamp(Cow.WrapAngle(this.cuttyDeadrot - this.cuttyRot), -0.06f, 0.06f));
      }
    }
    ...
    private void pigHealing(ref float[,] heights)
    {
      ...
      if ((double) this.healWait1 > 0.0)
      {
        this.fps = 0.4f;
        float height;
        Cow.GetHeightFast(ref heights, new Vector2(this.cuttyPos.X, this.cuttyPos.Z), out height);
    ...
    private void pigAttack(ref float[,] heights)
    {
      ...
      {
        this.attack1 = false;
        this.nextAttack = this.rr.Next(100 + Cow.cuttyCount * 150, Cow.cuttyCount * 320);
        this.newAction = false;
      }
      this.tx = this.destx;
      this.tz = this.destz;
    ...
    {
      int minValue1 = (int) (float) xmin;
      int minValue2 = (int) (float) ymin;
      int maxValue1 = (int) (float) xmax;
      int maxValue2 = (int) (float) ymax;
      ...
      if (this.targetChoice == 2)
      {
        ...
        this.spriteBatch.Draw(this.sc.wound, new Vector2((float) this.rr.Next(minValue1, maxValue1), (float) this.rr.Next(minValue2, maxValue2)), new Rectangle?(rectangle), color, (float) this.rr.Next(-800, 800) / 100f, new Vector2(32f, 32f), (float) this.rr.Next(90, 110) / 100f, SpriteEffects.None, 0.0f);
      ...
      int tech)
    {
      ...
      Matrix identity = Matrix.Identity;
      for (int index = 0; index < this.col_Bone.Length; ++index)
        this.DrawModel(this.sphere, Matrix.CreateScale(1f / 500f * this.col_Scale[index]) * (Matrix.CreateTranslation(this.col_Pos[index]) * this.a.skinTransforms[this.col_Bone[index]]));
    ...
    private void shockWave(ref float[,] heights)
    {
      ...
      for (float radians = 0.0f; (double) radians < 6.28000020980835; radians += num1)
      {
        ...
        Cow.GetHeightFast(ref heights, new Vector2(vector3_1.X, vector3_1.Z), out vector3_1.Y);
    ...
    {
      int index1 = (int) MathHelper.Clamp(position.X / Cow.unit, 0.0f, (float) (Cow.bitmap - 2));
      int index2 = (int) MathHelper.Clamp(position.Y / Cow.unit, 0.0f, (float) (Cow.bitmap - 2));
      float num1 = position.X % Cow.unit / Cow.unit;
      float num2 = position.Y % Cow.unit / Cow.unit;
    ...
    public struct hitStream : IVertexType
    {
      ...
      VertexDeclaration IVertexType.VertexDeclaration => Cow.hitStream.VertexDeclaration;
    }
    ...
    public struct instancedObject : IVertexType
    {
      ...
      {
        get => Cow.instancedObject.InstanceVertexDeclaration;
      }
    }
