...
namespace Blood
{
  internal class Cutty4
  {
    ...
    private Cutty4.hitStream hitstreamTemp = new Cutty4.hitStream();
    ...
    private Cutty4.instancedObject[] tempInstance = new Cutty4.instancedObject[1];
    private Cutty4.shell chunk;
    private Cutty4.shell faceChunk;
    private Cutty4.shell assChunk;
    ...
    public Cutty4.conductor tempConduct = new Cutty4.conductor();
    private Cutty4.conductor cuttyStruct = new Cutty4.conductor();
    private Cutty4.conductor cuttyStruct_send = new Cutty4.conductor();
    private List<Cutty4.conductor> scheduleList = new List<Cutty4.conductor>();
    ...
    public Cutty4(int index, int type, string name, int healthpoints)
    {
      Cutty4.shortestDistance = 20000f;
      Cutty4.bit = (byte) 0;
      Cutty4.uvIndex = (byte) 0;
      Cutty4.xcoord = 0;
      Cutty4.ycoord = 0;
      Cutty4.cuttyCount = 0;
      ...
      Cutty4.speeches = 0;
      Cutty4.whichPigTalks = 0;
      Cutty4.someoneTalking = false;
      Cutty4.gonnaHealindex = -1;
      Cutty4.homingLocal = -1;
      Cutty4.nameBuild.Length = 0;
      Cutty4.nameBuild.Append(name);
    ...
    public void LoadContent(ScreenManager sc, ContentManager cc)
    {
      ...
      this.fireball.stainTrans = new Cutty4.hitStream[this.fireball.stainCapacity];
      this.fireball.stainBuffer = new DynamicVertexBuffer(sc.GraphicsDevice, Cutty4.vd2, this.fireball.stainCapacity, BufferUsage.WriteOnly);
      ...
      this.chunk.buffer = new DynamicVertexBuffer(sc.GraphicsDevice, Cutty4.instanceDec, this.chunk.maxCapacity, BufferUsage.WriteOnly);
      this.chunk.displayList = new Cutty4.instancedObject[this.chunk.maxCapacity];
      ...
      this.chunk.stream = new Cutty4.instancedObject[this.chunk.maxCapacity];
      this.faceChunk = new Cutty4.shell();
      ...
      this.faceChunk.buffer = new DynamicVertexBuffer(sc.GraphicsDevice, Cutty4.instanceDec, this.faceChunk.maxCapacity, BufferUsage.WriteOnly);
      this.faceChunk.displayList = new Cutty4.instancedObject[this.faceChunk.maxCapacity];
      ...
      this.faceChunk.stream = new Cutty4.instancedObject[this.faceChunk.maxCapacity];
      this.assChunk = new Cutty4.shell();
      this.assChunk.max = 0;
      ...
      this.assChunk.buffer = new DynamicVertexBuffer(sc.GraphicsDevice, Cutty4.instanceDec, this.assChunk.maxCapacity, BufferUsage.WriteOnly);
      this.assChunk.displayList = new Cutty4.instancedObject[this.assChunk.maxCapacity];
      ...
      this.assChunk.stream = new Cutty4.instancedObject[this.assChunk.maxCapacity];
    ...
    public void SetTangents(ref Curve x, ref Curve y)
    {
      for (int index1 = 0; index1 < x.Keys.Count; ++index1)
      {
        ...
        Cutty4.SetCurveKeyTangent(ref key1, ref key3, ref key2);
        x.Keys[index1] = key3;
        CurveKey key4 = y.Keys[index2];
        CurveKey key5 = y.Keys[index3];
        CurveKey key6 = y.Keys[index1];
        Cutty4.SetCurveKeyTangent(ref key4, ref key6, ref key5);
        y.Keys[index1] = key6;
      }
    }
    ...
    {
      if (this.cuttyHeal || !Cutty4.cuttyDoneSpeech)
        return;
      this.health -= amt;
    ...
    public void checkTargets(Vector3 gunpos, Vector3 gunlook, ref Cursor genCursor, float wall)
    {
      ...
      else
      {
        ...
        if ((double) this.distanceCutty < 4000.0)
        {
          ...
          if (this.df == 3)
            amt += (ushort) 2;
          if (this.df > 2 && Cutty4.cuttyCount > 1)
            ++amt;
          if (this.df == 2)
            ++amt;
          if (this.df == 1)
      ...
      int bonehit)
    {
      ...
      Cutty4.uvIndex = (byte) this.myIndex;
      Cutty4.xcoord = (int) num2;
      Cutty4.ycoord = (int) num3;
    ...
    {
      this.talkIndex = 8;
      Cutty4.whichPigTalks = this.myIndex;
    ...
    private void dropChunk(ref Cutty4.shell sh, Matrix mm, int bone)
    {
    ...
    private void updateChunk(ref Cutty4.shell sh, ref float[,] heights)
    {
      sh.tempindex = 0;
      bool flag = true;
    ...
    private void addExplosion(ref Cutty4.hole hole, Vector3 campos)
    {
    ...
    private void updateExplosion(ref Cutty4.hole hole, Vector3 campos)
    {
    ...
    private void pigLogic()
    {
      ...
      {
        if (!Cutty4.allplayersReady || Cutty4.someoneTalking)
          return;
        if (Cutty4.speeches < this.speechList.Count && this.speechInclude.Contains(Cutty4.speeches))
        {
          Cutty4.someoneTalking = true;
          ...
          this.cuttyStruct.talkindex = this.speechList[Cutty4.speeches];
          this.scheduleAction(130);
          ++Cutty4.speeches;
        }
        else
        {
          if (Cutty4.speeches < this.speechList.Count)
            return;
      ...
      else
      {
        ...
        if (!this.newAction && (double) this.playerHealth != 100.0 && ((double) this.remoteHealth != 100.0 || this.spectator) && (double) this.healthPerc < 0.72000002861022949 && this.clipIndexA == 1 && this.nextAttack < 0 && this.homing == 0)
        {
          ...
          {
            num = this.distanceCutty;
            index1 = this.playerIndex;
            flag = true;
          }
          if (this.remplayer.Count > 1)
          {
            ...
            {
              if (index2 != this.playerIndex && (double) this.distanceCutty2[index2] >= 200.0 && (double) this.distanceCutty2[index2] <= 1500.0 && (double) this.distanceCutty2[index2] <= (double) num)
              {
                index1 = index2;
                num = this.distanceCutty2[index2];
                flag = true;
              }
            }
          }
          if (flag && (double) num >= 200.0 && (double) num <= 1500.0)
          {
            this.cuttyStruct.dur = (ushort) num;
            if (index1 == this.playerIndex)
            {
            ...
            {
              this.cuttyStruct.destx = this.remplayer[index1].r.lastPOS.X;
              this.cuttyStruct.destz = this.remplayer[index1].r.lastPOS.Z;
            }
        ...
        if (!this.newAction && Cutty4.gonnaHealindex == -1)
        {
          --this.gonnaHealDelay;
          float num1 = 700f;
          ...
          if (this.healCount > 0 && (flag1 || flag2) && (double) this.gonnaHealDelay <= 0.0)
          {
            ...
            float num2 = Vector3.Distance(this.roofTop1, this.cuttyPos);
            float y = this.roofTop1.Y;
            float num3 = num2;
            Vector3 vector3 = this.roofTop1;
            if ((double) num2 > (double) num1)
            {
              float num4 = Vector3.Distance(this.roofTop2, this.cuttyPos);
              y = this.roofTop2.Y;
              num3 = num4;
              ...
              {
                y = this.roofTop3.Y;
                float num5 = Vector3.Distance(this.roofTop3, this.cuttyPos);
            ...
            {
              --this.healCount;
              this.cuttyStruct.dur = (ushort) num3;
              this.cuttyStruct.destx = vector3.X;
              this.cuttyStruct.destz = vector3.Z;
              float num6 = (float) (-Math.Atan2((double) this.cuttyPos.Z - (double) vector3.Z, (double) this.cuttyPos.X - (double) vector3.X) - 1.5700000524520874);
              float num7 = Cutty4.WrapAngle(y - num6);
              float num8 = Cutty4.WrapAngle(y + 3.14f - num6);
              this.cuttyStruct.rot = (double) Math.Abs(num7) > (double) Math.Abs(num8) ? num6 + num8 : num6 + num7;
        ...
        if (this.curveIndex == curveIndex && this.homing <= 10)
          return;
        if (Cutty4.homingLocal == this.myIndex)
          Cutty4.homingLocal = -1;
        if (this.remplayer.Count > 1)
        {
          for (int index = 0; index < this.remplayer.Count; ++index)
          {
            if (index != this.playerIndex && this.remplayer[index].r.homingRemote == this.myIndex)
              this.remplayer[index].r.homingRemote = -1;
          }
        }
        int num9 = this.rr.Next(0, 1000);
        int num10 = this.rr.Next(0, this.remplayer.Count);
        if (this.spectator)
          num10 = this.playerIndex;
        int num11 = num10 + 1;
        if (this.homing > 10)
        {
          int index = this.homing - 11;
          num9 = index != this.playerIndex ? ((double) this.distanceCutty2[index] >= (double) this.rr.Next(100, 350) ? this.rr.Next(this.break1, 1000) : 1) : ((double) this.distanceCutty >= (double) this.rr.Next(100, 350) ? this.rr.Next(this.break1, 1000) : 1);
        }
        if (num9 < this.break1)
        {
          List<int> intList = new List<int>();
          if ((double) this.playerHealth > 0.0 && Cutty4.homingLocal == -1)
            intList.Add(this.playerIndex);
          if (this.remplayer.Count > 0)
          {
            for (int index = 0; index < this.remplayer.Count; ++index)
            {
              if (index != this.playerIndex && (double) this.remplayer[index].r.now.health > 0.0 && this.remplayer[index].r.homingRemote == -1)
                intList.Add(index);
            }
          }
          bool flag = true;
          if (intList.Count > 0)
          {
            num11 = intList[this.rr.Next(0, intList.Count)] + 1;
            flag = false;
          }
          if (this.homingCount >= 4 || flag)
            num9 = this.rr.Next(this.break1, 1000);
        }
        if (num9 >= this.break2)
        {
          this.homingCount = 0;
          ...
          if (this.rr.Next(1, 500) < 40 && this.quips.Count > 0 && this.myIndex == 0)
          {
            ...
            num12 = this.quips[index];
            this.quips.RemoveAt(index);
          }
          this.cuttyStruct.talkindex = num12;
          this.scheduleAction(130);
        }
        else if (num9 >= this.break1)
        {
          this.targetWait = this.rr.Next(150, 300);
          int num13 = -1;
          if (this.homingCount >= 1)
            num13 = this.rr.Next(9, 11);
          this.homingCount = 0;
          this.cuttyStruct.homing = 0;
          this.cuttyStruct.curveIndex = this.curveIndex;
          ...
          {
            int index = this.rr.Next(0, this.quips.Count);
            num13 = this.quips[index];
            this.quips.RemoveAt(index);
          }
          this.cuttyStruct.talkindex = num13;
          this.scheduleAction(130);
        }
        else
        {
          ...
          if (this.remplayer.Count > 1)
          {
            for (int index = 0; index < this.remplayer.Count; ++index)
            {
              if (this.cuttyStruct.homing - 1 == this.playerIndex)
                Cutty4.homingLocal = this.myIndex;
              if (this.cuttyStruct.homing - 1 == index)
                this.remplayer[index].r.homingRemote = this.myIndex;
            }
          }
          else
            Cutty4.homingLocal = this.myIndex;
          ...
          this.cuttyStruct.loop = this.loop;
          this.cuttyStruct.animType = 0;
          this.cuttyStruct.talkindex = num14;
          this.scheduleAction(130);
        }
      }
    }
    ...
    private void pigBones(Vector3 lookVec, ref float[,] heights)
    {
      ...
      this.distanceCutty2 = new float[6]
      {
        100000f,
        100000f,
        100000f,
      ...
      if (this.remplayer.Count > 1)
      {
        for (int index = 0; index < this.remplayer.Count; ++index)
        {
          if (index != this.playerIndex)
            this.distanceCutty2[index] = Vector3.Distance(this.remplayer[index].r.lastPOS, this.cuttyPos);
        }
      }
      if (this.attack1)
      ...
      if ((double) this.distanceCutty < 900.0 && this.health > (ushort) 0 && (this.homing == 0 || this.homing - 1 == this.playerIndex || this.homing - 11 == this.playerIndex))
      ...
      this.piglook = Cutty4.WrapAngle(this.piglook + MathHelper.Clamp(Cutty4.WrapAngle(num2 - this.piglook), -0.013f, 0.013f));
      ...
      if (!this.cuttyisDead)
      {
        ...
        if (!this.newAction)
        {
          ...
          else
          {
            bool flag1 = false;
            bool flag2;
            if (flag2 = this.homing > 10)
              this.homing -= 10;
            if (this.homing - 1 == this.playerIndex)
            {
              this.tx = this.playerpos.X;
              this.tz = this.playerpos.Z;
              num3 = Vector3.DistanceSquared(new Vector3(this.playerpos.X, this.playerpos.Y + 80f, this.playerpos.Z), vector3);
              if ((double) this.distanceCutty < 50.0 && (double) this.playerHealth > 105.0)
                this.cuttyAssPush = true;
              if (!(flag1 = (double) this.playerHealth <= 25.0) && !this.sc.host && !flag2 && (double) num3 < 10000.0 && this.animClip != 3)
                type = "bite";
            }
            else if (this.homing - 1 < this.remplayer.Count)
            {
              flag1 = (double) this.remplayer[this.homing - 1].r.now.health <= 25.0;
              this.tx = this.remplayer[this.homing - 1].r.lastPOS.X;
              this.tz = this.remplayer[this.homing - 1].r.lastPOS.Z;
              num3 = Vector3.DistanceSquared(new Vector3(this.remplayer[this.homing - 1].r.lastPOS.X, this.remplayer[this.homing - 1].r.lastPOS.Y + 80f, this.remplayer[this.homing - 1].r.lastPOS.Z), vector3);
            }
            else
            {
              this.tx = this.targetX[this.curveIndex].Evaluate(this.loop);
            ...
            if (!flag1 && this.sc.host && !flag2 && (double) num3 < 22500.0 && !this.gonnaBite)
            {
              this.targetWait = 90;
              this.gonnaBite = true;
              this.cuttyStruct.homing = this.homing + 10;
        ...
        if ((double) this.playerHealth > 25.0)
        {
          float num4 = Vector3.DistanceSquared(new Vector3(this.playerpos.X, this.playerpos.Y + 80f, this.playerpos.Z), vector3);
          if ((double) num4 < 10000.0 && this.animClip != 3)
          {
          ...
          if ((double) num4 < 22500.0 && this.animClip == 3)
          {
            this.cuttyMouthPos = new Vector2(vector3.X, vector3.Z) - new Vector2(this.playerpos.X, this.playerpos.Z);
            this.cuttyMouthCollide = true;
          }
        }
        float num5 = Cutty4.WrapAngle((float) (-Math.Atan2((double) this.cuttyPos.Z - (double) this.tz, (double) this.cuttyPos.X - (double) this.tx) - 1.5700000524520874) - this.cuttyRot);
        float max = 0.065f;
        if (this.homing > 0)
        {
          max = 0.065f;
          num5 = MathHelper.Clamp(num5, -0.065f, max);
          this.cuttyRot = Cutty4.WrapAngle(this.cuttyRot + num5);
        }
        if (this.attack1)
        {
          max = 0.06f;
        ...
        if (this.cuttyHeal)
        {
          ...
          {
            num5 = Cutty4.WrapAngle(this.cuttyHealrot - this.cuttyRot);
            max = 0.01f;
          }
        ...
        if (!this.death1 && !this.cuttyHeal && !this.attack1 && this.homing == 0)
          this.cuttyRot = Cutty4.WrapAngle(this.cuttyRot + MathHelper.Clamp(num5, -max, max));
      ...
      if (!this.attack1 && !this.cuttyHeal)
        Cutty4.GetHeightFast(ref heights, new Vector2(this.cuttyPos.X, this.cuttyPos.Z), out this.cuttyPos.Y);
    ...
    private void pigDying()
    {
      ...
      else
      {
        ...
        this.cuttyRot = Cutty4.WrapAngle(this.cuttyRot + MathHelper.Clamp(Cutty4.WrapAngle(this.cuttyDeadrot - this.cuttyRot), -0.06f, 0.06f));
      }
    }
    ...
    private void pigHealing(ref float[,] heights)
    {
      ...
      if ((double) this.healWait1 > 0.0)
      {
        this.fps = 0.4f;
        float height;
        Cutty4.GetHeightFast(ref heights, new Vector2(this.cuttyPos.X, this.cuttyPos.Z), out height);
    ...
    private void pigAttack(ref float[,] heights)
    {
      ...
      {
        this.attack1 = false;
        this.nextAttack = this.rr.Next(100 + Cutty4.cuttyCount * 150, Cutty4.cuttyCount * 320);
        this.newAction = false;
      }
      this.tx = this.destx;
      this.tz = this.destz;
    ...
    public void cuttyAction(ref Cutty4.conductor tempy, bool isLocal, ref float[,] heights)
    {
      if (tempy.flag == 130)
      {
        if (this.cuttyHeal)
      ...
      if (tempy.flag == 131)
      {
        ...
        Cutty4.GetHeightFast(ref heights, new Vector2(this.destx, this.destz), out this.desty);
      ...
      this.homing = 0;
      this.cuttyHeal = true;
      Cutty4.gonnaHealindex = this.myIndex;
      this.glowTimer = 1f;
      this.healtimer = 0.0f;
      ...
      Cutty4.GetHeightFast(ref heights, new Vector2(this.healdestx, this.healdestz), out this.healdesty);
    ...
    {
      Vector3 vector3 = new Vector3(0.3f, 0.3f, 0.6f) * (float) (Math.Sin((double) this.sc.myTimer / 30.0) / 2.0 + 0.5);
    ...
    private void shockWave(ref float[,] heights)
    {
      ...
      for (float radians = 0.0f; (double) radians < 6.28000020980835; radians += num1)
      {
        ...
        Cutty4.GetHeightFast(ref heights, new Vector2(vector3_1.X, vector3_1.Z), out vector3_1.Y);
    ...
    private void DrawGrenadeExplosion(ref Cutty4.hole hole, Matrix world)
    {
      int stainMax = hole.stainMax;
      if (stainMax < 1)
        return;
      ModelMeshPart meshPart = this.sc.fireballDecal.Meshes[0].MeshParts[0];
      hole.stainBuffer.SetData<Cutty4.hitStream>(hole.stainTrans, 0, stainMax, SetDataOptions.Discard);
    ...
    private void DrawInstance(Cutty4.shell shell, string tech)
    {
      int tempindex = shell.tempindex;
      if (tempindex < 1)
        return;
      ModelMeshPart meshPart = shell.model.Meshes[0].MeshParts[0];
      shell.buffer.SetData<Cutty4.instancedObject>(shell.displayList, 0, tempindex, SetDataOptions.Discard);
      Effect effect = meshPart.Effect;
    ...
    {
      int index1 = (int) MathHelper.Clamp(position.X / Cutty4.unit, 0.0f, (float) (Cutty4.bitmap - 2));
      int index2 = (int) MathHelper.Clamp(position.Y / Cutty4.unit, 0.0f, (float) (Cutty4.bitmap - 2));
      float num1 = position.X % Cutty4.unit / Cutty4.unit;
      float num2 = position.Y % Cutty4.unit / Cutty4.unit;
    ...
    public struct hitStream : IVertexType
    {
      ...
      VertexDeclaration IVertexType.VertexDeclaration => Cutty4.hitStream.VertexDeclaration;
    }
    ...
    public struct hole
    {
      ...
      public int[] bone;
      public Cutty4.hitStream[] stainTrans;
      public int[] frame;
      public Matrix[] scale;
    ...
    public struct instancedObject : IVertexType
    {
      ...
      {
        get => Cutty4.instancedObject.InstanceVertexDeclaration;
      }
    }
    ...
    public struct shell
    {
      ...
      public int[] bone;
      public Cutty4.instancedObject[] stream;
      public DynamicVertexBuffer buffer;
      public Cutty4.instancedObject[] displayList;
