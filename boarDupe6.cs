...
namespace Blood
{
  public class boarDupe6
  {
      ...
      bool noZboars)
    {
      ...
      this.isGrow = -1;
      if ((group == 1 && sc.int_7 == 1 || group == 2 && sc.int_8 == 1) && this.immunity < 5000 && this.random.Next(1, 100) < 70)
      {
        this.isGrow = 0;
      ...
      if (group == 1 && sc.int_2 > 0)
      {
        ...
        if (sc.int_2 == 1)
          this.speed = (float) (0.20000000298023224 * ((double) boarDupe6.acc / 1000.0)) * this.scaleSpeed;
        if (sc.int_2 == 2)
        {
          this.speed = (float) (0.20000000298023224 * ((double) boarDupe6.acc / 1000.0) * 7.0);
          this.scaleSpeed = 7f;
        }
        if (sc.int_2 == 3)
        {
          this.speed = (float) (0.20000000298023224 * ((double) boarDupe6.acc / 1000.0) * 10.0);
          this.scaleSpeed = 10f;
        }
        if (sc.int_2 == 4)
        {
          this.speed = (float) (0.20000000298023224 * ((double) boarDupe6.acc / 1000.0) * 22.0);
          this.scaleSpeed = 22f;
        }
      ...
      if (group == 1 && (formation > 19 && formation < 30 || (double) Math.Abs(sc.boar1Rot) >= 50.0))
      {
        ...
        float num2 = 6.28318548f * Vector2.Distance(vector2, new Vector2(startpos.X, startpos.Z)) / (this.speed * boarDupe6.handicapSpeed);
      ...
      if (group == 2 && (formation > 19 && formation < 30 || (double) Math.Abs(sc.boar2Rot) >= 50.0))
      {
        ...
        float num4 = 6.28318548f * Vector2.Distance(vector2, new Vector2(startpos.X, startpos.Z)) / (this.speed * boarDupe6.handicapSpeed);
    ...
    public void UpdateNormal(ref float[,] heights)
    {
      ...
      if (this.growTrigger)
      {
        ...
        if ((double) this.scale > (double) this.bluScale)
        {
          ...
          if (!this.undead)
          {
            this.isGrow = 2;
            if (this.boarGroup == 1)
              this.health = this.scale * ((float) boarDupe6.sc.int_9 / 10f);
            if (this.boarGroup == 2)
              this.health = this.scale * ((float) boarDupe6.sc.int_12 / 10f);
            if (this.homing != 0)
      ...
      this.mypos.X += (float) (-Math.Cos((double) this.myRot + 1.5707999467849731) * ((double) this.speed * (double) boarDupe6.handicapSpeed));
      this.mypos.Z += (float) Math.Sin((double) this.myRot + 1.5707999467849731) * (this.speed * boarDupe6.handicapSpeed);
      if ((double) this.mypos.X > 6900.0 || (double) this.mypos.X < -900.0)
        this.mypos.X = 6000f - this.mypos.X;
      if ((double) this.mypos.Z > 6900.0 || (double) this.mypos.Z < -900.0)
    ...
    public void UpdateDeath(ref float[,] heights)
    {
      ...
      if (this.undead)
        this.calTint();
      this.mypos.X += this.angleX * (this.speed * boarDupe6.handicapSpeed);
      this.mypos.Z += this.angleZ * (this.speed * boarDupe6.handicapSpeed);
      ...
      else if (this.isShocked == 2)
      {
          ...
          if (this.death && this.isHead <= 0 && !this.nozombies)
          {
            ...
            if (this.homing != 0)
            {
              ...
              if (this.random.Next(1, 100) < 51)
                this.ch = 3;
              this.speed = (float) ((double) this.scale * ((double) boarDupe6.acc / 1000.0) * (double) this.charge * 1.5);
        ...
        else
        {
          ...
          if (this.homing != 0)
          {
            ...
            if (this.random.Next(1, 100) < 51)
              this.ch = 3;
            this.speed = (float) ((double) this.scale * ((double) boarDupe6.acc / 1000.0) * (double) this.charge * 1.2000000476837158);
            this.scaleSpeed = this.charge;
          }
    ...
    {
      int num1 = (int) ((double) position.X / (double) boarDupe6.unit);
      int num2 = (int) ((double) position.Y / (double) boarDupe6.unit);
      int index1 = (int) MathHelper.Clamp(position.X / boarDupe6.unit, 0.0f, (float) (boarDupe6.bitmap - 2));
      int index2 = (int) MathHelper.Clamp(position.Y / boarDupe6.unit, 0.0f, (float) (boarDupe6.bitmap - 2));
